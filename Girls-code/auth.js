...
function setupAuthPage() {
    const currentPage = window.location.pat
// Authentication page functionality
import firebase from './firebase.js';
...
hname.split('/').pop();
    ...
    if (currentPage === 'login.html') {
        ...
        // Check if user is authenticated for gallery page
        firebase.onAuthStateChange((user) => {
            if (!user) {
                // Redirect to login page if not authenticated, passing gallery as redirect target
                window.location.href = `login.html?redirect=${encodeURIComponent('gallery.html')}`;
            }
        });
    }
    ...
    if (currentPage === 'login.html' || currentPage === 'signup.html') {
        firebase.onAuthStateChange((user) => {
            if (user) {
                ...
                const redirect = urlParams.get('redirect') || 'gallery.html';
...
async function handleGoogleLogin() {
    try {
        ...
        // Get the intended destination from URL params or default to gallery
        const urlParams = new URLSearchParams(window.location.search);
        const redirect = urlParams.get('redirect') || 'gallery.html';
        window.location.href = redirect;
    } catch (error) {
